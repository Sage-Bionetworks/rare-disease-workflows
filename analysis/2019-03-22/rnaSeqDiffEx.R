...
this.script='https://raw.githubusercontent.com/sgosline/NEXUS/master/analysis/2019-03-22/rnaSeqDiffEx.R'
...
lapply(names(comp.list),function(comp){
    ...
    diffex=full.res[which(full.res$padj<0.05),]%>%dplyr::select(gene,log2FoldChange)
    ...
    full.go<-summary(gg)%>%dplyr::select(ID,Description,GeneRatio,pvalue,p.adjust,qvalue,geneID)%>%mutate(genelist=paste(sapply(unlist(strsplit(geneID,split='/')),function(x) entrez_list[match(x,entrez_list$entrezgene),'gene']),collapse=','))%>%dplyr::select(-geneID)
